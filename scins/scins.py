...
def rdkit_mol_warning(mol):
    logging.warning(
        "Input should be an RDKit molecule object, but got an object of type %s. Therefore returning ERROR_SCINS" % type(
            mol))
...
def ring_list_to_num_macrocycles(ring_list):
    ...
    # macrocycles = [ring for ring in ring_list if len(ring) > 12]
    # if len(macrocycles) > 1:
    #
    # for now keep this as a boolean, for practical purposes (but we might want to change it to a number)
